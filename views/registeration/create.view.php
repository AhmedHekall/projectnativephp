                     ...
                     <div class="mt-5 sm:mx-auto sm:w-full sm:max-w-sm">
                            <form class="space-y-6" action="/register" method="POST">
                                   <div>
                                          ...
                                          <div class="mt-2">
                                                 <input type="text" value="<?= htmlspecialchars($_POST['firstname'] ?? ''); ?>" name="firstname" id="name"  required class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
                                          </div>
                                   </div>
                                   <?php if(isset($error['name'])) :?>
                                        <p class="text-red-400 text-xl mt-5"><?= $error['name'] ?></p> 
                                   <?php endif ?>      
                                   <div>
